...
rpart.plot(model_tree$finalModel) # plot tree: class D is absent from predictions
matrix_tree <- confusionMatrix(predictions_tree,traincv$classe)
matrix_tree # low accuracy, close if not worse than random
```
...
Use random forest model to generate final predictions on the test set. Out of sample error is estimated at .008 as per the below.

```{r results="markup"}
### FINAL PREDICTIONS
traincv$accurate <- predictions_rf == traincv$classe # assess accuracy of prediction
1-sum(traincv$accurate)/nrow(traincv) # out of sample error, MSRE not meaningful for factor prediction
